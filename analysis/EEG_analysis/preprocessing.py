...
eeg_dir = data_dir / 'experiment' / 'EEG' / subject_id / condition
...
outdir = eeg_dir / 'preprocessed' # create output directory
# subject_id = subfolder.name
if not outdir.exists():
    outdir.mkdir()
# collect header files
header_files = list(eeg_dir.glob('*.vhdr'))
raws = []
# concatenate blocks
...
ica.labels_["blinks"] = [1, 3]
ica.apply(epochs, exclude=ica.labels_["blinks"])
ica.save(outdir / f"{subject_id}_{condition}-ica.fif", overwrite=True)
del ica
