...
free_ears_fname = eeg_dir / 'Ears_Free_grand_average_conditions-ave.fif'
molds_fname = eeg_dir / 'Molds_grand_average_conditions-ave.fif'
molds_2_fname = eeg_dir / 'Molds_2_grand_average_conditions-ave.fif'
...
evoked_molds_2 = mne.read_evokeds(molds_2_fname)
molds_2_data = dict()
for evoked in evoked_molds_2:
    molds_2_data[evoked.comment] = evoked.pick_channels(picks).data[0]
...
fig, axis = plt.subplots(1,1, figsize=(13,4))
for i, key in enumerate(molds_data.keys()):
    axis.plot(times, molds_2_data[key],c=colors[i], label=key + '°')
ylim = axis.get_ylim()
axis.vlines(x=[-1, 0], ymin=ylim[0], ymax=ylim[1], color='black', linestyles='dashed', linewidth=.5)
axis.set_ylim(ylim[0], ylim[1])
axis.set_xlim(tmin, tmax)
...
axis.set_ylabel('Voltage (mV)')
axis.legend(title='Elevation')
axis.set_title(f'condition: adapted earmolds, channel: {picks[0]}')